...
// Parcelamento sem juros sobre o total da compra (R$ 46,54).
export const TOTAL_PAGAMENTO_CENTS = 4654;
export const OPCOES_PARCELAS = [1, 2, 3, 4, 6];

export default function CreditoParcelasScreen({ selectedIndex }: { selectedIndex: number }) {
  return (
        ...
        <div className="flex flex-col gap-[12px] w-[617px]">
          ...
          <div className="relative border border-[#e5e5e5] rounded-[20px] p-[20px]">
            <div className="fade-in-delay absolute bottom-full left-1/2 -translate-x-1/2 mb-[8px] w-[480px] pointer-events-none z-[45]">
              <div className="bg-[rgba(15,15,15,0.92)] flex gap-[16px] items-start px-[24px] py-[16px] rounded-[10px] shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/8" style={{ backdropFilter: 'blur(10px)' }}>
                ...
                <p className="font-['Nunito_Sans',sans-serif] text-[18px] text-[rgba(255,255,255,0.75)] leading-[1.6]" style={{ fontVariationSettings: "'YTLC' 500, 'wdth' 100" }}>
                  Selecione a forma de parcelamento desejada e pressione <span className="font-bold text-white">[Entra]</span> para confirmar a quantidade de parcelas.
                </p>
              </div>
            ...
            <div className="h-[195px] overflow-y-auto flex flex-col">
              {OPCOES_PARCELAS.map((n, idx) => {
                const ativo = idx === selectedIndex;
                const bg = ativo ? "bg-[#e4f6ff]" : idx % 2 === 1 ? "bg-[#f5f5f5]" : "bg-white";
                const valorParcela = Math.round(TOTAL_PAGAMENTO_CENTS / n);
                return (
                  <div key={n} className="flex items-center border-b border-[#e5e5e5]">
                    <div className={`shrink-0 w-[48px] px-[8px] py-[16px] ${bg}`}>
                      <p className="font-['Geist',sans-serif] text-[16px] text-[#0a0a0a] whitespace-nowrap">{n}x</p>
                    </div>
                    <div className={`flex-1 px-[8px] py-[16px] ${bg}`}>
                      <p className="font-['Geist',sans-serif] text-[16px] text-[#0a0a0a] whitespace-nowrap">
                        {n === 1 ? "(à Vista) 0+1" : `(Parcelado) 0+${n}`}
                      </p>
                    </div>
                    <div className={`shrink-0 px-[8px] py-[16px] ${bg}`}>
                      <p className="font-['Geist',sans-serif] text-[16px] text-[#0a0a0a] whitespace-nowrap">
